    ...
    return (React.createElement("div", { className: clsx(classes.root, className) },
        React.createElement("div", { className: classes.content },
            React.createElement("div", { className: classes.header },
                React.createElement("div", { className: classes.row }, columns.map((c, i) => (React.createElement(DataTableCell, { key: i, className: clsx(classes.cell, classes.headerTypography), width: c.width, fullWidth: i === columns.length - 1, title: c.description, align: c.align }, c.title || c.field))))),
            React.createElement("div", { className: classes.body }, error
                ? (React.createElement("div", { className: classes.row },
                    React.createElement(DataTableCell, { className: classes.errorTypography, fullWidth: true, align: 'center' }, errorLabel)))
                : (loading && dataSource.data.length === 0
                    ? (React.createElement("div", { className: classes.row },
                        React.createElement(DataTableCell, { className: classes.emptyTypography, fullWidth: true, align: 'center' }, loadingLabel)))
                    : (dataSource.data.length === 0
                        ? (React.createElement("div", { className: classes.row },
                            React.createElement(DataTableCell, { className: classes.emptyTypography, fullWidth: true, align: 'center' }, emptyLabel)))
                        : (dataSource.data.map((d, i) => (React.createElement("div", { key: i, className: clsx(classes.row, classes.bodyRow, selectable && selection === i && classes.selectedRow), onClick: () => onRowSelected(d, i) }, columns.map((c, j) => (React.createElement(DataTableCell, { key: j, className: i === dataSource.data.length - 1 ? undefined : classes.cell, width: c.width, fullWidth: j === columns.length - 1, align: c.align }, c.formatter ? c.formatter(d, c.field) : (c.field ? d[c.field] : '')))))))))))),
